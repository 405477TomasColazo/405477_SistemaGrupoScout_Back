...
public class EventRegistrationService {
    ...
    public List<EventRegistrationDTO> registerMembersToEvent(Long eventId, @NotEmpty(message = "Member IDs cannot be empty") List<Integer> memberIds) {
        ...
        for (Integer memberId : memberIds) {
            if (eventRegistrationRepository.existsByEventIdAndMemberId(eventId, memberId)) {
                throw new RuntimeException("Member with id " + memberId + " is already registered for this event");
            }

            Member member = memberRepository.findById(memberId)
                    .orElseThrow(() -> new RuntimeException("Member not found with id: " + memberId));

            if (event.getMaxCapacity() != null && event.getCurrentCapacity() >= event.getMaxCapacity()) {
    ...
    public EventRegistrationDTO updateRegistrationStatus(Long eventId, Long registrationId, @NotNull(message = "Status is required") RegistrationStatus status) {
        if (!eventRepository.existsById(eventId)) {
            throw new RuntimeException("Event not found with id: " + eventId);
        }
    ...
    public EventRegistrationDTO respondToInvitation(Long eventId, Integer userId, RegistrationStatus status) {
        Event event = eventRepository.findById(eventId)
                .orElseThrow(() -> new RuntimeException("Event not found with id: " + eventId));
        ...
        Member member = memberRepository.findByUser(user)
                .orElseThrow(() -> new RuntimeException("Member record not found for user: " + userId));

        // Check if already registered
        EventRegistration existingRegistration = eventRegistrationRepository
                .findByEventIdAndMemberId(eventId, member.getId()).orElse(null);
        ...
        if (existingRegistration != null) {
            ...
            if (status == RegistrationStatus.CONFIRMED && oldStatus != RegistrationStatus.CONFIRMED) {
                event.setCurrentCapacity(event.getCurrentCapacity() + 1);
                eventRepository.save(event);
            } else if (status != RegistrationStatus.CONFIRMED && oldStatus == RegistrationStatus.CONFIRMED) {
                event.setCurrentCapacity(event.getCurrentCapacity() - 1);
                eventRepository.save(event);
            }
            ...
            // Create new registration if accepting invitation
            if (status == RegistrationStatus.CONFIRMED) {
                ...
                // If declining, just return a response without creating registration
                throw new RuntimeException("Invitation declined");
            }
        }
    }
